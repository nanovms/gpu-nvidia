...
static inline unsigned long find_last_bit(const unsigned long *addr, u64 nbits)
{
    if (nbits) {
        u64 word_offset = (nbits - 1) >> 6;
        u64 w = addr[word_offset] & MASK(nbits - 1 - (word_offset << 6));
        while (1) {
            u64 bit = msb(w);
            if (bit != INVALID_PHYSICAL)
                return (word_offset << 6) + bit;
...
#define PageDirty(p)            true
#define SetPageDirty(p)
#define ClearPageDirty(p)
#define TestClearPageDirty(p)   true
